...
class NewsService {
  ...
  async getTrendingTopics(): Promise<TrendingTopic[]> {
    ...
    const topics = new Map<string, { count: number; category: string; views: number }>();

    articles.forEach(({ title, category, views }) => {
      const words = title.toLowerCase().split(/\s+/);
      words.forEach((word: string) => {
        if (word.length > 3) {
          const current = topics.get(word) || { count: 0, category, views: 0 };
          topics.set(word, {
            count: current.count + 1,
            category,
            views: current.views + (views || 0)
          });
        }
      });
    });
    ...
    return Array.from(topics.entries())
      .map(([topic, { count, category, views }]) => ({
        topic,
        count,
        category,
        views
